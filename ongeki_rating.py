...
async def getAvatar(avatar) -> Image.Image:
    async with aiohttp.ClientSession() as sess:
        try:
            ...
            return Image.open(ONGEKI_DIR / 'cover' / 'fallback.webp', 'rb')
...
async def getCover(song: MusicInfo) -> Image.Image:
    for s in music_list:
        if s["title"] == song.name:
            try:
                return Image.open(ONGEKI_DIR / 'cover_ori' / s["imageName"]).convert('RGBA')
...
class Draw:
    basic = Image.open(ONGEKI_DIR / 'pattern_basic.png')
    advanced = Image.open(ONGEKI_DIR / 'pattern_advanced.png')
    expert = Image.open(ONGEKI_DIR / 'pattern_expert.png')
    master = Image.open(ONGEKI_DIR / 'pattern_master.png')
    ...
    async def whiledraw(self, data: List[Rating], height: int = 0) -> None:
        ...
        for num, info in enumerate(data):
            ...
            rate = Image.open(ONGEKI_DIR / f'score_{SCORE_RANKS[info.playlog.tech_score_rank - 1]}.png').resize((95, 44))
            ...
            fc = Image.open(ONGEKI_DIR / fc_img).resize((120, 36))
            self._im.alpha_composite(fc, (x + 146, y + 82))
            fb = Image.open(ONGEKI_DIR / fb_img).resize((120, 36))
            self._im.alpha_composite(fb, (x + 268, y + 82))

            title = info.music.name
            if coloumWidth(title) > 22:
...
class DrawBest(Draw):
    ...
    async def draw(self) -> Image.Image:
        ...
        rating_number = Image.open(ONGEKI_DIR / 'rating' / f'num_{rating_index}.png')
        rating_numbers = []
        for j in range(4):
            for i in range(4):
                rating_numbers.append(rating_number.crop((34*i, 36*j, 34*(i+1), 36*(j+1))))

        logo = Image.open(ONGEKI_DIR / 'logo.png').convert('RGBA').resize((410, 190))
        rating_header = Image.open(ONGEKI_DIR / 'rating' / f'header_{rating_index}.png').resize((158, 42))
        rank = Image.open(ONGEKI_DIR / 'rating' / f'rank_{rank_index}.png')
        rank_bg = Image.open(ONGEKI_DIR / 'rating' / f'rank_bg_{rank_bg_index}.png').resize((130, 280))
        level = Image.open(ONGEKI_DIR / 'rating' / 'level_bg.png')
        name_bg = Image.open(ONGEKI_DIR / 'name_bg.png')
        rating_bg = Image.open(ONGEKI_DIR / 'extra_bg.png').resize((454, 50))

        self._im.alpha_composite(logo, (4, 112))

        plate = Image.open(ONGEKI_DIR / 'plate.png').resize((1420, 230))
        self._im.alpha_composite(plate, (390, 100))
        icon = Image.open(ONGEKI_DIR / 'icon_bg.png').resize((214, 214))
        ...
        for n, i in enumerate(rating_str):
            if n < 2:
                ...
                self._im.alpha_composite(rating_numbers[12].resize((34, 37)), (860, 280))
            else:
                self._im.alpha_composite(rating_numbers[int(i)].resize((34, 37)), (805 + 25 * n, 280))
